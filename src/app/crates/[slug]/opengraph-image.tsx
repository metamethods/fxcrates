...
export default async function OpenGraph({
  ...
  const crateResult = await fetchCrate(slug);
  if (!crateResult) return new Response('Not Found', { status: 404 });
  const owners = await getOwners(slug);

  return new ImageResponse(
    (
    <div tw="flex w-full h-full">
      <div tw="bg-white w-full h-full flex flex-col justify-between p-16">
        <div tw="flex items-start justify-between gap-8">
          <div tw="flex flex-col items-start">
            {/* TODO: make {crateResult.name} bolded */}
            <h1 tw="text-zinc-800 text-6xl font-bold">
              crates/{crateResult.name}
            </h1>
            <p tw="text-zinc-600 text-3xl">{crateResult.description}</p>
            <div tw="flex">
              {owners.map((owner, index) => {
                ...
                return (
                  <div tw="flex" key={index}>
                    <img src={owner.avatar!} alt="Avatar" tw={`w-16 h-16 rounded-lg ${index !== 1 ? 'ml-2' : ''}`} />
                  </div>
                );
              })}
            </div>
          </div>
          <img src={owners[0].avatar!} alt="Avatar" tw="w-64 h-64 rounded-lg" />
        </div>
        <div tw="flex items-center justify-between w-full">
          <div tw="flex">
            <div tw="flex items-center">
              ...
              <p tw="text-zinc-700 text-3xl leading-none ml-2">
                {crateResult.downloads}
              </p>
            </div>
      ...
      <div 
        tw="absolute flex bottom-0 left-0 w-full h-3" 
        style={{
          backgroundColor: '#dda584'
        }}
      />
    </div>
    ),
